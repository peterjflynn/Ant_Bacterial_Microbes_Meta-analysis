...
setwd("/Users/peterflynn/Documents/Ant_Bacterial_Review_Myrmeco/Revision_analysis/")
#### richness estimates from Kass 2022####
library(terra)
library(sf)

# Step 1: Load the raster using the correct file path
rf_richness <- rast("ants.rf.plus.irs.hs.tif")
...
ant_review_data <- read_csv("ant_review_spreadsheet_nosanger_Chansonadded_1_play.csv")
...
# Load the original data
ant_review_data <- read_csv("ant_review_spreadsheet_sanger_ngs_Chansonadded_play.csv")
...
test <- read.csv("ant_review_spreadsheet_sanger_ngs_Chansonadded_play.csv")
...
# Load and clean the dataset
test <- read.csv("ant_review_spreadsheet_nosanger_Chansonadded_1_play.csv")
test$Bacterial_order1 <- trimws(tolower(test$Bacterial_order1))
test$Bacterial_order2 <- trimws(tolower(test$Bacterial_order2))
test$Bacterial_order3 <- trimws(tolower(test$Bacterial_order3))
test$Diet <- trimws(test$Diet)
...
# Custom pastel palette for Country (more saturated pastel shades)
country_palette <- c("#E69F00", "#56B4E9", "#009E73","#999999", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
...
diet_levels <- unique(long_data_clean$Diet)

# Create named palette using Okabe-Ito (subset if necessary)
diet_palette <- setNames(okabe_ito[seq_along(diet_levels)], diet_levels)
